...
**HTML chunks.** `add /body --type htmlchunk --prop html='<h2>…</h2><table>…</table>'` (or `--prop src=page.html`) embeds HTML verbatim as a Word `altChunk`; Word converts it to native content when the file is opened. It stays one opaque `/body/altChunk[N]` node until then (preview shows escaped source, text is not queryable), so prefer `--type markdown` / native adds when you need to edit or verify the content afterwards. `officecli materialize file.docx` does that conversion headless for HTML/XHTML/plain text: data-URI png, jpeg, gif, bmp, tiff, emf and wmf become inline drawings (alt text is the picture description); webp, svg, and http(s) or relative image URLs stay as alt text and do not fail the chunk. CSS borders on a paragraph, callout, or table cell become `w:pBdr` / `w:tcBorders` (`solid`, `dashed`, `dotted`, `double`, `inset`, `outset`; `groove`/`ridge` are approximated). Inline borders, `border-radius`, and `border-image` are ignored. A data URI that cannot be decoded is the same warning and does not trip `--strict`. A nested `ul`/`ol` inside an `li` shares one `numId` (`ilvl` is the depth); a separate list, including one in a table cell, gets its own `numId`. Also takes `format=rtf|text|xhtml|mht` and `matchSrc=true`; `help docx htmlchunk`.
